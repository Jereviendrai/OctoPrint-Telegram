...
class TMSG():
	...
	def _sendNotification(self, payload, **kwargs):
		...
		try:
			...
			message = self.main._settings.get(["messages",kwargs['event'],"text"]).format(emo,**locals()).encode("utf-8")
		except Exception as ex:
			self._logger.error("Exception on formatting message: " + str(ex))
			message =  self.main.gEmo('warning') + " ERROR: I was not able to format the Notification for '"+event+"' properly. Please open your OctoPrint settings for " + self.main._plugin_name + " and check message settings for '" + event + "'." + str(ex)
